 ...
 * GSS
 * 
 * This was the GSS software to be run on every Feather.
 * Development was stopped around Spring break 2020
 * 
 * This program would run on every Feather and ID 1 would be the GSM
 * Both the GSU and GSM would go to sleep for 10 seconds and wake up via interrupt from the built in RTC.
 * The GSU would then send a message out and wait for a reply. It would continue to send a message
 * until a reply was heard.
 * The GSM would wake up and listen for 10 seconds, if it received a message, it would check the first
 * byte for '1' so that it knew it was intended for it. Then reply.
 */
